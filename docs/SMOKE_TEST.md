...
5. Move the prepared dish between a cooker, carrier, and plate. Verify the source transfer neither duplicates nor loses the canonical physical count. Remove or serve it and verify all compatibility coverage disappears immediately, all tints clear, and every otherwise-eligible guess returns.
6. Use the same ingredients with a different cooking-step ID, raw progress, burnt progress, and an incorrect mixing state. Verify none is covered; then use the correct cooking step and completed cooking or mixing progress and verify it is.
...
6. Verify `Track by player / 按玩家批量勾选` appears above the family buttons with Player 1-4 counts `15, 12, 15, 13`. Confirm the five Fruit Platters belong to both Player 3 and Player 4 and the unique union remains exactly 50 recipes.
